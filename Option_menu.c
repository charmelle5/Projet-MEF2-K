...
void rechercher_animal(Animal* chenil, int taille) {
	...
	int choix;
    char nom_recherche[MAXCHAR] = {0};
    int espece = 0, type_age = 0;
    const int annee_actuelle = 2025;
	...
	switch(choix) {
        ...
        case 2:
            printf("Entrez le numero de l'espece (1:Chien, 2:Chat, 3:Hamster, 4:Autruche, 5:Lapin, 6:Poisson): ");
            if (scanf("%d", &espece) != 1 || espece < 1 || espece > 6) {
        ...
        case 3:
            printf("Rechercher :\n1. Jeune (<2 ans)\n2. Senior (>10 ans)\nVotre choix : ");
            if (scanf("%d", &type_age) != 1 || (type_age != 1 && type_age != 2)) {
                printf("Choix invalide.\n");
                vider_buffer();
                return;
            }
	...
	for (int i = 0; i < taille; i++) {
		...
		switch(choix) {
		case 1:
			if (strstr(chenil[i].nom, nom_recherche)) {
                    match = 1;
                }
			break;
		case 2:
            if (chenil[i].espece == espece) {
                    match = 1;
                }
			break;
		case 3:
			int age = annee_actuelle - chenil[i].annee;
                if ((type_age == 1 && age < 2) || (type_age == 2 && age > 10)) {
                    match = 1;
                }
			break;
	...
	if (nb_resultats == 0) {
        printf("Aucun animal ne correspond aux criteres.\n");
        return;
    }
    ...
    for (int i = 0; i < nb_resultats; i++) {
        printf("\n--- Animal %d/%d ---\n", i+1, nb_resultats);
        afficher_animal(chenil[resultats[i]]);
    }
